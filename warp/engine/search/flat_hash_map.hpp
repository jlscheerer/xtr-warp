...
#ifndef BAT_WARP_ENGINE_SEARCH_FLAT_HASH_MAP
#define BAT_WARP_ENGINE_SEARCH_FLAT_HASH_MAP
...
public:
  ...
  sherwood_v8_table() {}
  explicit sherwood_v8_table(size_type bucket_count,
  ...
  sherwood_v8_table(size_type bucket_count, const ArgumentAlloc &alloc)
      : sherwood_v8_table(bucket_count, ArgumentHash(), ArgumentEqual(),
                          alloc) {}
  sherwood_v8_table(size_type bucket_count, const ArgumentHash &hash,
  ...
  sherwood_v8_table(It first, It last, size_type bucket_count = 0,
                    const ArgumentHash &hash = ArgumentHash(),
                    const ArgumentEqual &equal = ArgumentEqual(),
                    const ArgumentAlloc &alloc = ArgumentAlloc())
  ...
  sherwood_v8_table(It first, It last, size_type bucket_count,
                    const ArgumentAlloc &alloc)
      : sherwood_v8_table(first, last, bucket_count, ArgumentHash(),
                          ArgumentEqual(), alloc) {}
  template <typename It>
  sherwood_v8_table(It first, It last, size_type bucket_count,
                    const ArgumentHash &hash, const ArgumentAlloc &alloc)
      : sherwood_v8_table(first, last, bucket_count, hash, ArgumentEqual(),
                          alloc) {}
  ...
  sherwood_v8_table(std::initializer_list<T> il, size_type bucket_count,
                    const ArgumentAlloc &alloc)
      : sherwood_v8_table(il, bucket_count, ArgumentHash(), ArgumentEqual(),
                          alloc) {}
  sherwood_v8_table(std::initializer_list<T> il, size_type bucket_count,
                    const ArgumentHash &hash, const ArgumentAlloc &alloc)
  ...
  sherwood_v8_table(sherwood_v8_table &&other,
  ...
  sherwood_v8_table &operator=(sherwood_v8_table &&other) noexcept {
    if (this == std::addressof(other))
      ...
      AssignIfTrue<
          ...
          *this, std::move(other));
      swap_pointers(other);
    } else if (static_cast<ByteAlloc &>(*this) ==
               static_cast<ByteAlloc &>(other)) {
      swap_pointers(other);
    } else {
      clear();
      _max_load_factor = other._max_load_factor;
  ...
  public:
    ...
    templated_iterator &operator++() {
      do {
        ...
        if (index-- == 0)
          break;
      } while (current->control_bytes[index % BlockSize] ==
    ...
    ValueType &operator*() const { return current->data[index % BlockSize]; }
    ValueType *operator->() const { return current->data + index % BlockSize; }
  ...
  iterator end() { return {entries - 1, std::numeric_limits<size_t>::max()}; }
  ...
  inline std::pair<iterator, bool> emplace(Key &&key, Args &&...args) {
    ...
    for (;;) {
      ...
      int8_t to_next_index = metadata & Constants::bits_for_distance;
      if (to_next_index == 0)
        return emplace_new_key({index, block}, std::forward<Key>(key),
                               std::forward<Args>(args)...);
      index += Constants::jump_distances[to_next_index];
      index = hash_policy.keep_in_range(index, num_slots_minus_one);
    }
  ...
  void rehash(size_t num_items) {
    num_items = std::max(
        num_items, static_cast<size_t>(std::ceil(
  ...
  convertible_to_iterator erase(const_iterator to_erase) {
    ...
    if (current.has_next()) {
      ...
      AllocatorTraits::construct(*this, std::addressof(*current),
                                 std::move(*next));
  ...
  iterator erase(const_iterator begin_it, const_iterator end_it) {
    ...
    if (begin_it.current->control_bytes[begin_it.index % BlockSize] ==
        Constants::magic_for_empty)
...
private:
  ...
  struct LinkedListIt {
    ...
    bool is_empty() const { return metadata() == Constants::magic_for_empty; }
    bool has_next() const { return jump_index() != 0; }
    int8_t jump_index() const {
      return Constants::distance_from_metadata(metadata());
    }
    int8_t metadata() const { return block->control_bytes[index_in_block()]; }
  ...
  emplace_direct_hit(LinkedListIt block, Args &&...args) {
    ...
    if (block.metadata() == Constants::magic_for_empty) {
      AllocatorTraits::construct(*this, std::addressof(*block),
                                 std::forward<Args>(args)...);
      ...
      for (LinkedListIt it = block;;) {
        AllocatorTraits::construct(*this, std::addressof(*free_block.second),
                                   std::move(*it));
...
class bytell_hash_map
  ...
  using Table = detailv8::sherwood_v8_table<
      std::pair<K, V>, K, H, detailv8::KeyOrValueHasher<K, std::pair<K, V>, H>,
...
public:
  ...
  friend bool operator==(const bytell_hash_map &lhs,
                         const bytell_hash_map &rhs) {
...
class bytell_hash_set : public detailv8::sherwood_v8_table<
                            T, T, H, detailv8::functor_storage<size_t, H>, E,
                            detailv8::functor_storage<bool, E>, A,
                            typename std::allocator_traits<
                                A>::template rebind_alloc<unsigned char>,
...
public:
  ...
  friend bool operator==(const bytell_hash_set &lhs,
                         const bytell_hash_set &rhs) {
    if (lhs.size() != rhs.size())
      return false;
...
#endif /* BAT_WARP_ENGINE_SEARCH_FLAT_HASH_MAP */
