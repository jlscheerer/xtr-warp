...
template<int8_t nbits>
torch_annotated_stride_view<> parallel_decompress_centroids(
  ...
  at::parallel_for(0, nprobe * num_query_tokens, 1, [&](int64_t begin, int64_t end) {
    for (int cell_idx = begin; cell_idx < end; ++cell_idx) {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("parallel_decompress_centroids_2_cpp", &parallel_decompress_centroids<2>, "Decompress Centroid Embeddings (nbits=2)");
  m.def("parallel_decompress_centroids_4_cpp", &parallel_decompress_centroids<4>, "Decompress Centroid Embeddings (nbits=4)");
}